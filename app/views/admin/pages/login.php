...
if ($_SERVER["REQUEST_METHOD"] === "POST" && isset($_POST["login"])) {
    ...
    // Database sınıfını kullanarak bağlantıyı al
    $db = new Database();
    $pdo = $db->connect();
    ...
    if ($user && password_verify($password, $user["password"])) {
        // Kullanıcı giriş yaptığında status'ü 1 olarak güncelle
        $updateStmt = $pdo->prepare("UPDATE users SET status = 1 WHERE id = :id");
        ...
        $errors["login"] = "Geçersiz giriş bilgileri";
        $_SESSION["errors"] = $errors;
        header("Location: index.php");
        exit();
    }
...
<div class="page">
    ...
    ?>
        ...
        <div class="card card-md">
            <div class="card-body">
                <h2 class="h2 text-center mb-4">Hesabınızı Giriniz</h2>
                <form  method="POST"  autocomplete="off" novalidate>
                    <div class="mb-3">
                        <label class="form-label">E-Posta</label>
                        <input name="email" type="email" class="form-control" placeholder="eposta@email.com" autocomplete="off">
                    </div>
                    <div class="mb-2">
                        ...
                        <div class="input-group input-group-flat">
                            <input name="password" type="password" class="form-control" placeholder="Şifreniz" autocomplete="off">
